...
Ext.define('Jhako.controller.common.Sleepjob', {
  ...
  onSaveRecord: function() {
    ...
    var panel = tab.child('#comm_sleepjob');
    var form_jobunit = panel.child('#comm_jobunit');
    var form_sleepjob = panel.child('#sleepjob_detail');
    var rec_jobunit = form_jobunit.getRecord();
    var rec_sleepjob = form_sleepjob.getRecord();

    // Save
    form_jobunit.updateRecord();
    form_sleepjob.updateRecord();
    rec_sleepjob.save();
  ...
  onLoadRecord: function(record) {
    ...
    Jhako.model.Jobunit.load(record.data.id, {
      success: function(rec) {
        var rec_sleepjob = rec['Jhako.model.SleepjobHasOneInstance'];
        if (rec_sleepjob) {
          rec_sleepjob.getProxy().url = location.pathname + '/jobunits/' + rec.data.id + '/sleepjobs';
          JhakoSetForm(form_sleepjob, rec_sleepjob);
        }
      }
    });
  },
  ...
  onLoadProcRecord: function(record) {
    ...
    JhakoSetForm(form_jobunit, record);
    JhakoSetForm(form_sleepjob, record['Jhako.model.SleepjobHasOneInstance']);
  },
